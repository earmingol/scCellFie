"""Rank Metabolic Tasks using Scanpy functions for ranking genes through differential expression."""
...
def rank_metabolic_tasks_groups(
    adata: AnnData,
    groupby: str,
    ...
    **kwds,
    ...
    >>> sc.tl.rank_metabolic_tasks_groups(adata, 'bulk_labels', method='wilcoxon')
    >>> # to visualize the results
    >>> sc.pl.rank_metabolic_tasks_groups(adata)
    """
    if use_raw is None:
...
def filter_rank_metabolic_tasks_groups(
    adata: AnnData,
    key=None,
    groupby=None,
    use_raw=None,
    ...
    >>> adata = sc.datasets.pbmc68k_reduced()
    >>> sc.tl.rank_metabolic_tasks_groups(adata, 'bulk_labels', method='wilcoxon')
    >>> sc.tl.filter_rank_metabolic_tasks_groups(adata, min_fold_change=3)
    >>> # visualize results
    >>> sc.pl.rank_metabolic_tasks_groups(adata, key='rank_genes_groups_filtered')
    ...
    for cluster in gene_names.columns:
        ...
        if not use_logfolds:
            ...
            fold_change_matrix.loc[:, cluster] = np.log2(
                (mean_in_cluster + 1e-9)
                / (mean_out_cluster + 1e-9)
            )
    ...
    gene_names = gene_names[
        ...
        & (mean_in_cluster_matrix >= min_mean_value)
    ]
