...
def test_number_of_markers():
    n_markers = 5
    adata = create_random_adata()
    markers = quick_markers(adata, cluster_key='cluster', n_markers=n_markers, fdr=1.1)
...
def test_expression_cutoff():
    express_cut = 10
    adata = create_random_adata()
    markers1 = quick_markers(adata, cluster_key='cluster', fdr=1.1)
    markers2 = quick_markers(adata, cluster_key='cluster', express_cut=express_cut, fdr=1.1)
...
def test_cell_groups():
    adata = create_random_adata()
    markers = quick_markers(adata, cluster_key='cluster', cell_groups=['cluster1', 'cluster2', 'cluster3'], fdr=1.1)
