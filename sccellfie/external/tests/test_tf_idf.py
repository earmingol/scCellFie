...
def test_filter_tfidf_ratio(sample_marker_df):
    ...
    # Check if all remaining entries meet the ratio threshold
    assert np.all(filtered_df['tf'] / filtered_df['second_best_tf'] > tf_ratio)
...
def test_filter_tfidf_combined(sample_marker_df):
    ...
    # Check if all remaining entries meet both thresholds
    assert np.all(filtered_df['tf_idf'] > tfidf_threshold)
    assert np.all(filtered_df['tf'] / filtered_df['second_best_tf'] > tf_ratio)
